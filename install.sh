  ...
  if ! have "$cmd"; then
    ...
    if [ "$PM" != "none" ]; then
      if ! install_pkg "$PM" "$cmd"; then
        echo "⚠️  Failed to install $cmd automatically. Please install it and re-run."
      fi
    else
      echo "⚠️  No supported package manager detected. Please install $cmd manually."
    fi
  fi
done

if ! have zsh; then
  echo "📦 zsh not found; attempting to install (used as default shell)"
  if [ "$PM" != "none" ]; then
...
if [ -x "$DEST_TPM/bin/install_plugins" ]; then
  ...
  if ! "$DEST_TPM/bin/install_plugins"; then
    echo "⚠️  Plugin install encountered an issue. You can retry inside tmux with prefix + I."
  fi
else
  echo "ℹ️  TPM install script not found. Inside tmux, press prefix + I to install plugins."
fi

echo "✅ All set! Start tmux with: tmux"
